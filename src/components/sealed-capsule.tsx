...
export function SealedCapsule({
  ...
  return (
    <div className="flex flex-1 items-center justify-center bg-linear-to-b from-amber-50 via-rose-50 to-stone-100 px-6 py-16">
      <main className="w-full max-w-lg overflow-hidden rounded-[2rem] border border-amber-100/80 bg-white/85 shadow-xl shadow-amber-900/10">
        <div className="relative h-56 bg-linear-to-br from-amber-100 via-rose-100 to-stone-200">
          {coverUrl ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img
              src={coverUrl}
              alt=""
              className="size-full object-cover blur-[6px] scale-110"
            />
          ) : null}
          <div className="absolute inset-0 bg-stone-900/40" />
          <div className="absolute inset-0 flex flex-col items-center justify-center px-6 text-center">
            <p className="rounded-full bg-white/15 px-3 py-1 text-[11px] tracking-[0.35em] text-amber-50 uppercase backdrop-blur-sm">
              sealed
            </p>
            <h1 className="mt-4 text-3xl font-semibold text-white drop-shadow-sm">
              {recipient}에게
            </h1>
            <p className="mt-2 text-sm text-amber-50/80">아직 기간이 남았어요</p>
          </div>
        </div>
        ...
        <div className="px-8 py-9">
          ...
          <p className="mt-6 text-center text-sm leading-6 text-stone-400">
            열람일이 되기 전에는 편지와 사진 {imageCount}장을 볼 수 없어요.
